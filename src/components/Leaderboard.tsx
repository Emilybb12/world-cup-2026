...
function PlayerCard({ name, picks }: { name: string; picks: PlayerPicks }) {
  ...
  return (
    <div className="border border-field-600 bg-field-800 flex-1 min-w-[220px]">
      <div className="px-6 py-5 border-b border-field-700 flex items-center justify-between">
        <h3 className="font-display font-800 text-3xl tracking-wide text-white uppercase">{name}</h3>
        <p className={`font-display font-700 text-2xl ${pct === 100 ? 'text-gold-400' : 'text-field-400'}`}>
          {pct}<span className="text-sm">%</span>
        </p>
      </div>

      <div className="px-6 pt-4 pb-2">
        <div className="h-px bg-field-600 relative">
          <div className="absolute top-0 left-0 h-px bg-gold-500 transition-all duration-700"
            style={{ width: `${pct}%` }} />
        </div>
      </div>

      <div className="grid grid-cols-3 divide-x divide-field-700 border-b border-field-700">
        {[{ label: 'Groups', value: `${groupsDone}/12` }, { label: 'WC', value: `${picks.wildcard_picks.length}/8` }, { label: 'R32', value: `${r32Done}/16` }]
          .map(({ label, value }) => (
            <div key={label} className="px-3 py-3 text-center">
              ...
              <p className="text-xs tracking-widest uppercase text-field-300 mt-0.5">{label}</p>
            </div>
          ))}
      </div>
      <div className="grid grid-cols-3 divide-x divide-field-700 border-b border-field-700">
      ...
      <div className="px-6 py-5">
        <p className="text-xs font-display tracking-widest uppercase text-field-300 mb-2">🏆 Champion</p>
        {champion ? (
          <p className="font-display font-700 text-xl tracking-wide text-white uppercase">
            {TEAM_FLAGS[champion] ?? ''} {champion}
          </p>
        ) : (
          <p className="font-display text-lg text-field-500 uppercase">Not yet picked</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-field-200 tracking-wider">Completion &amp; champion picks</p>
      </div>

      {allAgree && (
        <div className="mb-8 border-2 border-gold-500 bg-gold-500 px-8 py-4 flex items-center gap-3">
          <span className="text-2xl">🤝</span>
          <p className="font-display font-700 text-lg tracking-wide text-field-900 uppercase">
      ...
      <div>
        <p className="text-xs font-display font-600 tracking-widest uppercase text-field-300 mb-4">
          Group Picks — Side by Side
        </p>
        <div className="overflow-x-auto border border-field-600">
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="border-b border-field-600 bg-field-850">
                <th className="text-left py-3 px-4 font-display tracking-widest uppercase text-field-300 font-600">Group</th>
                <th className="py-3 px-4 font-display tracking-widest uppercase text-field-300 font-600 text-left">Em 1st</th>
                <th className="py-3 px-4 font-display tracking-widest uppercase text-field-300 font-600 text-left">Em 2nd</th>
                <th className="py-3 px-4 font-display tracking-widest uppercase text-field-300 font-600 text-left border-r border-field-600">Em WC</th>
                <th className="py-3 px-4 font-display tracking-widest uppercase text-field-300 font-600 text-left">Ro 1st</th>
                <th className="py-3 px-4 font-display tracking-widest uppercase text-field-300 font-600 text-left">Ro 2nd</th>
                <th className="py-3 px-4 font-display tracking-widest uppercase text-field-300 font-600 text-left">Ro WC</th>
              </tr>
            </thead>
            <tbody>
              {GROUPS.map((group, gi) => {
                ...
                return (
                  <tr key={group.id} className={`border-b border-field-700 ${gi % 2 === 0 ? 'bg-field-800' : 'bg-field-850'}`}>
                    <td className="py-3 px-4 font-display font-700 tracking-wider text-white uppercase">
                      Group {group.id}
                    </td>
                    {[em.first, em.second, em.third].map((t, j) => (
                      <td key={j} className="py-3 px-4 text-field-100">
                        {t ? `${TEAM_FLAGS[t] ?? ''} ${t}` : <span className="text-field-500">—</span>}
                      </td>
                    ))}
                    {[ro.first, ro.second, ro.third].map((t, j) => (
                      <td key={j} className={`py-3 px-4 text-field-100 ${j === 0 ? 'border-l border-field-600' : ''}`}>
                        {t ? `${TEAM_FLAGS[t] ?? ''} ${t}` : <span className="text-field-500">—</span>}
                      </td>
                    ))}
                  </tr>
