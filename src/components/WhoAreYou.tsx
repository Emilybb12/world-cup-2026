...
export function WhoAreYou({ onSelect }: Props) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <p className="text-xs font-display tracking-widest uppercase text-field-300 mb-2">
        World Cup Predictions
      </p>
      <h1 className="font-display font-800 text-6xl tracking-wide text-white uppercase mb-2">
        Ro & Em
      </h1>
      <p className="text-sm text-field-300 tracking-wider mb-12">Who are you?</p>
      ...
      <div className="flex gap-3 w-full max-w-xs">
        {(['ro', 'em'] as Player[]).map((p) => (
          <button
            key={p}
            onClick={() => onSelect(p)}
            className="flex-1 py-6 border border-field-600 bg-field-800 hover:border-gold-500 hover:bg-gold-500 transition-all duration-200 group"
          >
            <span className="font-display font-800 text-3xl tracking-widest text-white uppercase group-hover:text-field-900 transition-colors">
              {p === 'em' ? 'Em' : 'Ro'}
            </span>
          </button>
        ))}
