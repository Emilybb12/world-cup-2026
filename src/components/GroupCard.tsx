...
export function GroupCard({ group, pick, onPick }: Props) {
  ...
  return (
    <div className={[
      ...
      {/* Row — click to toggle */}
      <button
        className="w-full flex items-center gap-4 px-5 py-4 hover:bg-navy-750 transition-colors text-left"
        onClick={() => setOpen((o) => !o)}
      >
        {/* Group letter */}
          ...
          {isComplete ? (
            <div className="flex items-center gap-4 flex-wrap">
              {[pick.first, pick.second, pick.third].map((team, i) => (
                <div key={i} className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-navy-100 hidden sm:block font-400">{team}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="flex gap-2 flex-wrap">
              {group.teams.map((team) => (
                <span key={team} className="text-xs text-navy-300 font-400">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className={`text-navy-400 text-[10px] transition-transform duration-200 ${open ? 'rotate-180' : ''}`}>▼</span>
        </div>
      </button>
        ...
        <div className="border-t border-navy-600 px-5 py-4">
              ...
              return (
                <div key={team} className="flex items-center gap-3">
                  <div className="flex items-center gap-2 w-44 shrink-0">
                    ...
                    <span className="text-sm text-navy-100 font-400 truncate">{team}</span>
                  </div>
                        ...
                        <button
                          ...
                          className={[
                            ...
                            isSelected
                              ...
                              : 'border-navy-600 text-navy-200 hover:border-gold-400 hover:text-gold-300 cursor-pointer',
                          ].join(' ')}
                        >
                          {short}
                        </button>
          ...
          {isComplete && (
            <button
              ...
              className="mt-4 w-full py-2 border border-gold-500 text-gold-400 text-xs font-display font-700 tracking-widest uppercase hover:bg-gold-500 hover:text-navy-900 transition-all"
            >
              Done ✓
            </button>
          )}
