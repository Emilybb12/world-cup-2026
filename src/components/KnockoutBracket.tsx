...
function MatchCard({ match, round, groupPicks, wildcardPicks, knockoutPicks, onPick }: MatchCardProps) {
  ...
  return (
    <div className={['border overflow-hidden', canPick ? 'border-ink-200 bg-white' : 'border-ink-100 bg-off-50'].join(' ')}>
        ...
        return (
          <button
            ...
            className={[
              ...
              idx === 0 ? 'border-b border-ink-100' : '',
              isWinner ? 'bg-ink-900 text-white font-500'
                : isLoser ? 'text-ink-200'
                : team ? 'text-ink-600 hover:bg-off-50 hover:text-ink-900 cursor-pointer'
                : 'text-ink-200 cursor-default',
            ].join(' ')}
          >
            <span>{team ? (TEAM_FLAGS[team] ?? '🏳') : ''}</span>
            <span className="truncate font-400">{team ?? '—'}</span>
            {isWinner && <span className="ml-auto text-white/50 text-xs">✓</span>}
          </button>
        );
      })}
    </div>
...
function RoundColumn({ label, matches, round, groupPicks, wildcardPicks, knockoutPicks, onPick }: {
  ...
  return (
    <div className="flex flex-col min-w-[156px]">
      <p className="text-xs font-display font-600 tracking-widest uppercase text-ink-400 mb-3 text-center">{label}</p>
...
export function KnockoutBracket({ groupPicks, wildcardPicks, knockoutPicks, onWildcardChange, onKnockoutPick }: Props) {
  ...
  if (!allGroupsDone) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        <p className="font-display font-800 text-4xl text-ink-200 uppercase mb-2">Locked</p>
        <p className="text-xs tracking-widest uppercase text-ink-200">Complete all 12 groups first</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-8">
        <h2 className="font-display font-800 text-4xl tracking-wide text-ink-900 uppercase mb-1">Knockout Bracket</h2>
        <p className="text-sm text-ink-400 tracking-wider">Click a team to advance them to the next round</p>
      </div>

      <WildcardPicker groupPicks={groupPicks} wildcardPicks={wildcardPicks} onChange={onWildcardChange} />

      {wildcardPicks.length < 8 && (
        <p className="mb-8 text-center text-xs tracking-widest uppercase text-ink-200 font-display">
          Select 8 wildcards to unlock the bracket
        </p>
      )}

      {champion && (
        <div className="mb-8 border-2 border-ink-900 bg-ink-900 px-8 py-5 flex items-center gap-4">
          <span className="text-2xl">🏆</span>
          <div>
            <p className="text-xs font-display tracking-widest uppercase text-white/50 mb-0.5">Champion</p>
            <p className="font-display font-800 text-2xl tracking-wide text-white uppercase">
      ...
      <div className="overflow-x-auto pb-4">
        <div className="flex gap-3 min-w-max">
          ...
          <div className="flex flex-col min-w-[156px]">
            <p className="text-xs font-display font-600 tracking-widest uppercase text-ink-400 mb-3 text-center">{ROUND_LABELS.final}</p>
